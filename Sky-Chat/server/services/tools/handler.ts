...
export function parseToolCalls(toolCalls: ToolCall[]): ParsedToolCall[] {
  return toolCalls.map((call) => {
    ...
    try {
      ...
      console.error(
        `[ToolHandler] Failed to parse arguments for ${call.function.name}:`,
        error
      )
    }
...
export function formatToolMessage(result: ToolCallResult): ToolMessage {
  ...
  if (result.name === 'web_search' || result.name === 'knowledge_search' || result.name === 'memory_search') {
    try {
...
export async function executeToolCall(
  ...
  console.log(
    `[ToolHandler] Executing tool: ${parsedCall.name}`,
